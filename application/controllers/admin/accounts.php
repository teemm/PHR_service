...
class Accounts extends Base {
    ...
    public function get_contents() {
        try {
            ...
            $query = 'SELECT c.id, cs.name content_status_name, '.'c.title_ka, c.title_en '
                .' FROM phr_content c '
                .' INNER JOIN phr_menu_content mc ON mc.content_id=c.id AND mc.menu_id ='.ACCOUNTS_MENU_ID
    ...
    public function save_content() {
        try {
            ...
            if ($this->upload->do_upload('userfile')) {
                ...
                $img_path = 'assets/uploads/accounts/' . $upload_data['file_name'];
                // Insert or update account in database
                $this->save_content_to_database($img_path);
            } else {
                // Status: image upload failed
                // (1) we are updating account and want to leave old image
    ...
    private function save_content_to_database($img_path = NULL) {
        ...
        if ($content_id) {
            ...
            if ($img_path) {
                ...
                // we have to copy old image link and update accounts
                $content_model['image'] = $old_img;
            }
        } else {
            // we are inserting
            if (!$img_path) {
                // we have to insert new accounts with default image
                $content_model['image'] = 'assets/uploads/accounts/default.png';
            }
            // else we don't change anything and insert accounts with new image
        }
